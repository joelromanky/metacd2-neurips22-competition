...
TIME_LIMIT = 10000 # time limit of the whole process in seconds
...
from losses import ContrastiveLoss, DistillKL, contrast_distill
...
class MyMetaLearner(MetaLearner):
    ...
    def __init__(self, 
        ...
        self.model = Wrapper(rn_timm_mix(True, 'swsl_resnet50', 
            0.1)).to(DEVICE)
        self.teacher = Wrapper(rn_timm_mix(True, 'swsl_resnet50', 
            0.1)).to(DEVICE)
    ...
    def meta_fit(self, 
        ...
        #criterion_contrast = ContrastiveLoss(temperature=10.0)
        ...
        while self.timer.time_left() > 60 * 5:
            ...
            for _ in range(5):
                ...
                for i, batch in enumerate(meta_train_generator(10)):
                    ...
                    # Student Forward
                    spatial_feat, avg_feat = self.model(X_train, is_contrast=True)
                    logit = self.cls(avg_feat)
                    global_feat = self.projector(avg_feat)

                    # Teacher Forward
                    with torch.no_grad():
                      spatial_t, avg_feat_t = self.teacher(X_train_aug, is_contrast=True)
                    ...
                    # Div loss
                    loss_div = criterion_div(logit, logits_t) / 10.
                    ...
                    #loss_contrast_global = criterion_contrast(global_feat, labels=y_train) / 10.
                    # loss_contrast_spatial = criterion_contrast_spatial(spatial_feat,
                    #                                                     labels=y_train,
                    #                                                     attention=attention) / 10.

                    # Total loss
                    loss_contrast = 10.*loss_contrast_global + 0.*loss_contrast_spatial
                    loss = .5*loss_contrast + .5*loss_div + 1.*loss_ce
